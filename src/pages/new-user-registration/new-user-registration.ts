...
@Component({
  selector: 'page-new-user-registration',
  templateUrl: 'new-user-registration.html'
})
export class NewUserRegistrationPage {
  ...
  createPerson(firstName: string, lastName: string, email: string, phoneNumber: string, height: string, weight: string, password: string, confirmPass: string): void { //Adds new user to database
    ...
    if (!fname.test(firstName) || firstName == null || !lname.test(lastName) || lastName == null || !email.endsWith(ending) || email == null || phoneNumber == null || !num.test(phoneNumber) || height == null || weight == null || password == null || !confirmPass.match(password)) {
      let toast = toastMessage.create({
